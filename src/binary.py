...
def encode_binary(ast: AstType) -> bytes:
    ...
    # - Compressed data
    header = (
        MAGIC_NUMBER + struct.pack("<B", FORMAT_VERSION) + struct.pack("<I", checksum)
    )
...
def decode_binary(data: bytes) -> AstType:
    ...
    if stored_checksum != computed_checksum:
        raise ValueError(
            f"Checksum mismatch: file may be corrupted (expected {stored_checksum:08x}, got {computed_checksum:08x})"
        )
