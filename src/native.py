...
class X86Compiler:
    ...
    def emit_runtime_call(self, func_name: str):
        ...
        self._emit_aligned_call(func_name)

    def _emit_aligned_call(self, func_name: str):
        """Emit a 16-byte aligned call to an external function."""
    ...
    def _emit_external_call(self, func_name: str):
        """Emit a platform-appropriate aligned call to a non-runtime external function."""
        if IS_WINDOWS:
            self.emit("push rax  # Save rax and check alignment")
            self.emit("mov rax, rsp")
            self.emit("add rax, 8  # Account for the push")
            ...
            self.emit("pop rax")
            self.emit(f"jz .L{func_name}_aligned_{self.label_counter}")
            self.emit("sub rsp, 40  # Shadow space + alignment")
            self.emit(f"call {func_name}")
                ...
                if line.strip() == "main:":
                    ...
                    if stack_size_match := re.search(r'sub rsp, (\d+)', line):
                        ...
                        init_code.extend([
                            ...
                            f"    lea rdi, [rip + {filename_label}]  # filename",
                            f"    lea rsi, [rip + {source_label}]  # source",
                            "    push rax  # Save rax and check alignment",
                            "    mov rax, rsp",
                            "    add rax, 8  # Account for the push",
                            "    test rax, 0xF",
    ...
    def _compile_entry(self, args: List[str]):
        ...
        if entry_point != 'main':
            ...
            if self.source_file and self.source_lines_dict:
                ...
                self.emit(f"lea rdi, [rip + {filename_label}]  # filename")
                self.emit(f"lea rsi, [rip + {source_label}]  # source")
    ...
    def _compile_const_f64(self, args: List[str]):
        ...
        for value in args:
            ...
            self.emit(f"movsd xmm0, [rip + {label}]")
            self.emit("sub rsp, 8")
    ...
    def _compile_div_f64(self, args: List[str]):
        ...
        self.emit_runtime_call("runtime_check_div_zero_f64_at")
        self.emit("add rsp, 8")
    ...
    def _compile_store_const_f64(self, args: List[str]):
        ...
        for i in range(0, len(args), 2):
            if i + 1 < len(args):
                ...
                self.emit(f"movsd xmm0, [rip + {label_name}]")
    ...
    def _compile_div_const_i64(self, args: List[str]):
        ...
        if int(value) == 0:
            ...
            self.emit(f"lea rdi, [rip + {message_label}]  # error message")
            self.emit(f"mov rsi, {self.current_line}  # line number")
            self.emit_runtime_call("runtime_error_at")
    ...
    def _compile_list_get(self, args: List[str]):
        ...
        self.emit_runtime_call("runtime_list_get_int_at")
    ...
    def _compile_list_set(self, args: List[str]):
        ...
        self.emit_runtime_call("runtime_list_set_int_at")
        self.emit("mov rax, [rip + list_append_scratch]")
    ...
    def _compile_list_new_f64(self, args: List[str]):
        ...
        for value in values:
            ...
            self.emit(f"movsd xmm0, [rip + {label}]")  # value in xmm0
    ...
    def _compile_list_new_str(self, args: List[str]):
        ...
        for value_str in values:
            ...
            self.emit("mov rdi, [rsp]")  # Peek at list pointer
            self.emit(f"lea rsi, [rip + {label}]")  # value as string pointer
            self.emit_runtime_call("runtime_list_append_int")
    ...
    def _compile_load_global(self, args: List[str]):
        ...
        self.emit(f"mov rax, [rip + global_vars + {var_index * 8}]")
        self.emit("push rax")
    ...
    def _compile_store_global(self, args: List[str]):
        ...
        var_index = int(args[0])
        self.emit("pop rax")
        self.emit(f"mov [rip + global_vars + {var_index * 8}], rax")
    ...
    def _compile_try_begin(self, args: List[str]):
        ...
        self.emit(f"lea rdi, [rip + {exc_type_label}]  # exc_type")
        self.emit_runtime_call("runtime_exception_push")

        # Get the jump buffer and save the current context with setjmp
        self.emit_runtime_call("runtime_exception_get_jump_buffer")
        if IS_WINDOWS:
            self.emit("mov rcx, rax  # jump buffer pointer")
        else:
            self.emit("mov rdi, rax  # jump buffer pointer")
        self._emit_external_call("setjmp")
    ...
    def _compile_try_end(self, args: List[str]):
        ...
        self.emit_runtime_call("runtime_exception_pop")
    ...
    def _compile_raise(self, args: List[str]):
        ...
        self.emit(f"lea rdi, [rip + {exc_type_label}]")
        self.emit(f"lea rsi, [rip + {message_label}]")
        self.emit(f"mov rdx, {self.current_line}  # line number")
        self.emit_runtime_call("runtime_exception_raise_at")
