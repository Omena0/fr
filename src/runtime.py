...
def format_runtime_exception(e: Exception) -> str:
    ...
    # Format location
    location = (
        f"{ctx['file']}:{ctx['line']}:{char_pos}"
        if ctx["file"]
        else f"Line {ctx['line']}:{char_pos}"
    )
...
def run_func(
    name: str, args, level=0
) -> int | float | bool | str | None | dict | list | Any:
    ...
    if func["type"] == NODE_TYPE_BUILTIN and callable(func["func"]):
        ...
        raise RuntimeError(
            f'Cannot call C function "{name}" from Python runtime. Use C VM or Native compiler.'
        )
    else:
        func_body = func["func"]
    ...
    if isinstance(target, str) and target in vars:
        ...
        if hasattr(obj, "__dict__") or hasattr(obj, field_name):
            ...
            try:
                ...
                raise RuntimeError(
                    f"Python object has no attribute '{field_name}'"
                ) from e
    ...
    if "func" in node or ("name" in node and "type" in node and node["type"] == "call"):
        ...
        for arg in args:
            ...
            if (
                "slice" in arg
                or "attr" in arg
                or ("value" in arg and isinstance(arg.get("value"), dict))
            ):
...
def eval_expr_calc(left, op, right):
    ...
    if op == "Eq":
        ...
        try:
            ...
            # Best-effort column info: point at the first '/' on the current source line
            global _runtime_current_char
            if _runtime_source and _runtime_current_line > 0:
                try:
                    source_line = _runtime_source.split("\n")[_runtime_current_line - 1]
                    pos = source_line.find("/")
                    if pos >= 0:
                        _runtime_current_char = pos
                except Exception:
                    pass

            # Raise as ZeroDivisionError so try/except "ZeroDivisionError" can catch it.
            # Use the message expected by the test suite.
            raise ZeroDivisionError("float division by zero") from e
    elif op in ["Pow", "**"]:
        return left**right
    elif op in ["Mod", "%"]:
        return left % right
...
def _handle_loop_control(
    e: BreakException | ContinueException, level_threshold: int = 1
):
...
def _execute_node_struct_def(node: dict):
    ...
    vars[node["name"]] = {"type": NODE_TYPE_STRUCT_DEF, "fields": node["fields"]}
...
def _execute_node_py_import(node: dict):
    ...
    if name:
        ...
        key = alias or name
        py_imports[key] = {"module": module_name, "type": "name", "name": name}
    else:
        # "py_import module" or "py_import module as alias"
        key = alias or module_name
        py_imports[key] = {"module": module_name, "type": "module"}
...
def _execute_node_var(node: dict):
    ...
    if isinstance(value, dict):
        ...
        # Check if it's a runtime expression (has slice, attr, or other runtime features)
            ...
            "slice" in value
            or "attr" in value
            or ("values" in value and "value" not in value)
        ):
            value = eval_expr_node(value)
    ...
    vars[node["name"]] = {"type": node["value_type"], "value": value}
...
def _execute_node_index_assign(node: dict):
    ...
    if isinstance(target, list):
        if index < 0 or index >= len(target):
            raise RuntimeError(
                f"Index {index} out of range for list of length {len(target)}"
            )
        target[index] = value
    else:
        raise RuntimeError(f"Cannot index into type {type(target).__name__}")
...
def _execute_node_for(node: dict, level: int):
    ...
    step_val = (
        ensure_int(eval_expr(node.get("step", 1)), "Step value")
        if "step" in node
        else 1
    )
...
def _execute_node_for_in(node: dict, level: int):
    ...
    iterable = (
        eval_expr_node(iterable_expr)
        if isinstance(iterable_expr, dict)
        else iterable_expr
    )
    ...
    if not isinstance(iterable, (list, str)):
        raise RuntimeError(
            f"Cannot iterate over non-iterable type: {type(iterable).__name__}"
        )
...
def run(ast: AstType, file: str = "", source: str = ""):
    ...
    with _runtime_lock:
        ...
        user_func_names = [
            ...
            and funcs[name].get("type") != "c_function"
        ]
...
__all__ = ["run", "eval_expr", "run_scope", "vars", "runtime"]
