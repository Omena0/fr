...
class TypeStack(list):
    ...
    def pop(self, index=-1):
        if (
            hasattr(self.compiler, "extended_type_stack")
            and self.compiler.extended_type_stack
        ):
    ...
    def __setitem__(self, index, value):
        ...
        if (
            hasattr(self.compiler, "extended_type_stack")
            and len(self.compiler.extended_type_stack) > index
        ):
...
class WasmCompiler:
    ...
    def __init__(self):
        ...
        self.struct_type_stack: List[Optional[int]] = (
            []
        )  # Track struct IDs on the value stack (parallel to type_stack)
        ...
        self.local_value_types: Dict[int, str] = (
            {}
        )  # Track what type of value each local holds (set, list, bool)
        self.builtin_map = {
            ...
            "set_timeout": (
                ...
                ["i32"],
                [],
            ),  # Returns i32, params handled specially
        ...
        # Exception handling
        self.try_stack: List[Tuple[str, str]] = (
            []
        )  # Stack of (error_type, handler_label)
        self.global_str_bases: Set[int] = (
            set()
        )  # Base indices for string globals (ptr, len)
        ...
        self.timer_signatures: Dict[str, str] = (
            {}
        )  # Maps timer func name to param signature

        # Optional source lines (used to compute columns for runtime errors)
        self._source_lines_for_columns: Optional[List[str]] = None
    ...
    def compile(
        self, bytecode: str, source_file: Optional[str] = None
    ) -> Tuple[str, Dict]:
        """Compile bytecode to WebAssembly text format (WAT)"""
        if source_file:
            try:
                self._source_lines_for_columns = (
                    Path(source_file).read_text(encoding="utf-8").splitlines()
                )
            except Exception:
                self._source_lines_for_columns = None
    ...
    def _collect_imports_used(self, lines: List[str]):
        ...
        for line in lines:
            ...
            # Process management opcodes (not CALL-based)
            if line.startswith("FORK"):
                self.imports.add("fork")
                continue
            if line.startswith("WAIT") or line.startswith("JOIN"):
                self.imports.add("wait")
                continue
    ...
    def _emit_imports(self):
        ...
        if "dom_get_body" in self.imports:
            self.emit(
                '(import "env" "dom_get_body" (func $dom_get_body (result i32)))', 1
            )
        if "dom_get_document" in self.imports:
            self.emit(
                '(import "env" "dom_get_document" (func $dom_get_document (result i32)))',
                1,
            )
        ...
        if "dom_get_text" in self.imports:
            self.emit(
                '(import "env" "dom_get_text" (func $dom_get_text (param i32) (result i32 i32)))',
                1,
            )
        if "dom_set_html" in self.imports:
            self.emit(
                '(import "env" "dom_set_html" (func $dom_set_html (param i32 i32 i32)))',
                1,
            )
        if "dom_get_html" in self.imports:
            self.emit(
                '(import "env" "dom_get_html" (func $dom_get_html (param i32) (result i32 i32)))',
                1,
            )
        if "dom_set_attr" in self.imports:
            self.emit(
                '(import "env" "dom_set_attr" (func $dom_set_attr (param i32 i32 i32)))',
                1,
            )
        if "dom_get_attr" in self.imports:
            self.emit(
                '(import "env" "dom_get_attr" (func $dom_get_attr (param i32 i32 i32) (result i32 i32)))',
                1,
            )
        if "dom_remove_attr" in self.imports:
            self.emit(
                '(import "env" "dom_remove_attr" (func $dom_remove_attr (param i32 i32 i32)))',
                1,
            )
        if "dom_append" in self.imports:
            self.emit(
                '(import "env" "dom_append" (func $dom_append (param i32 i32)))', 1
            )
        if "dom_prepend" in self.imports:
            self.emit(
                '(import "env" "dom_prepend" (func $dom_prepend (param i32 i32)))', 1
            )
        if "dom_remove" in self.imports:
            self.emit('(import "env" "dom_remove" (func $dom_remove (param i32)))', 1)
        if "dom_clone" in self.imports:
            self.emit(
                '(import "env" "dom_clone" (func $dom_clone (param i32 i32) (result i32)))',
                1,
            )
        if "dom_parent" in self.imports:
            self.emit(
                '(import "env" "dom_parent" (func $dom_parent (param i32) (result i32)))',
                1,
            )
        if "dom_children" in self.imports:
            self.emit(
                '(import "env" "dom_children" (func $dom_children (param i32) (result i32)))',
                1,
            )
        ...
        if "dom_remove_class" in self.imports:
            self.emit(
                '(import "env" "dom_remove_class" (func $dom_remove_class (param i32 i32 i32)))',
                1,
            )
        if "dom_toggle_class" in self.imports:
            self.emit(
                '(import "env" "dom_toggle_class" (func $dom_toggle_class (param i32 i32 i32)))',
                1,
            )
        if "dom_has_class" in self.imports:
            self.emit(
                '(import "env" "dom_has_class" (func $dom_has_class (param i32 i32 i32) (result i32)))',
                1,
            )
        ...
        if "dom_get_style" in self.imports:
            self.emit(
                '(import "env" "dom_get_style" (func $dom_get_style (param i32 i32 i32) (result i32 i32)))',
                1,
            )
        if "dom_get_value" in self.imports:
            self.emit(
                '(import "env" "dom_get_value" (func $dom_get_value (param i32) (result i32 i32)))',
                1,
            )
        if "dom_set_value" in self.imports:
            self.emit(
                '(import "env" "dom_set_value" (func $dom_set_value (param i32 i32 i32)))',
                1,
            )
        ...
        if "dom_query_all" in self.imports:
            self.emit(
                '(import "env" "dom_query_all" (func $dom_query_all (param i32 i32) (result i32)))',
                1,
            )
        if "dom_on" in self.imports:
            self.emit(
                '(import "env" "dom_on" (func $dom_on (param i32 i32 i32 i32)))', 1
            )
        if "dom_off" in self.imports:
            self.emit('(import "env" "dom_off" (func $dom_off (param i32)))', 1)
        if "event_prevent_default" in self.imports:
            self.emit(
                '(import "env" "event_prevent_default" (func $event_prevent_default))',
                1,
            )
        if "event_stop_propagation" in self.imports:
            self.emit(
                '(import "env" "event_stop_propagation" (func $event_stop_propagation))',
                1,
            )
        if "event_target" in self.imports:
            self.emit(
                '(import "env" "event_target" (func $event_target (result i32)))', 1
            )
        ...
        if "set_timeout" in self.imports:
            ...
            self.emit(
                f'(import "env" "set_timeout" (func $set_timeout {sig} (result i32)))',
                1,
            )
        if "set_interval" in self.imports:
            sig = self.timer_signatures.get("set_interval", "(param i32 i32)")
            self.emit(
                f'(import "env" "set_interval" (func $set_interval {sig} (result i32)))',
                1,
            )
        if "clear_timeout" in self.imports:
            self.emit(
                '(import "env" "clear_timeout" (func $clear_timeout (param i32)))', 1
            )
        if "clear_interval" in self.imports:
            self.emit(
                '(import "env" "clear_interval" (func $clear_interval (param i32)))', 1
            )
        ...
        # Import runtime error reporting (error_type_ptr, error_type_len, message_ptr, message_len, line_num, col_num)
        self.emit(
            '(import "env" "runtime_error" (func $runtime_error (param i32 i32 i32 i32 i32 i32)))',
            1,
        )

        # Process management (only if used)
        if "fork" in self.imports:
            self.emit('(import "env" "fork" (func $fork (result i64)))', 1)
        if "wait" in self.imports:
            self.emit('(import "env" "wait" (func $wait (param i64) (result i64)))', 1)
        ...
        self.emit(
            '(import "env" "list_get" (func $list_get (param i32 i64) (result i64)))', 1
        )
        self.emit(
        ...
        self.emit(
            '(import "env" "list_len" (func $list_len (param i32) (result i64)))', 1
        )
        self.emit(
            '(import "env" "list_pop" (func $list_pop (param i32) (result i32 i64)))', 1
        )
        self.emit(
        ...
        self.emit(
            '(import "env" "set_add" (func $set_add (param i32 i64) (result i32)))', 1
        )
        self.emit(
        ...
        self.emit(
            '(import "env" "set_len" (func $set_len (param i32) (result i64)))', 1
        )

        # Math functions
        self.emit(
            '(import "env" "round_f64" (func $round_f64 (param f64) (result f64)))', 1
        )
        self.emit(
            '(import "env" "floor_f64" (func $floor_f64 (param f64) (result f64)))', 1
        )
        self.emit(
            '(import "env" "ceil_f64" (func $ceil_f64 (param f64) (result f64)))', 1
        )
        ...
        self.emit(
            '(import "env" "file_write" (func $file_write (param i32 i32 i32)))', 1
        )
    ...
    def _emit_globals(self):
        ...
        for idx, var_type in sorted(self.global_vars.items()):
            ...
            self.emit(
                f"(global $g{idx} (mut {wasm_type}) ({wasm_type}.const {init_value}))",
                1,
            )
    ...
    def _compile_function(
        self, func_name: str, func_meta: Dict, bytecode_lines: List[str]
    ):
        """Compile a single function"""
        self.current_function = func_name
        self.local_vars = func_meta["locals"].copy()
        self.type_stack = TypeStack(self)
        self.extended_type_stack = (
            []
        )  # Track detailed types (e.g. 'str') parallel to type_stack
        self.struct_type_stack = []
        self.label_stack = []

        # Source line tracking for runtime_error / raise
        self.current_line = 0

        # TRY/EXCEPT lowering support (catch_type, handler_label)
        self.try_stack = []
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("CALL dom_set_text") or line.startswith(
                "CALL dom_set_html"
            ):
            ...
            if (
                line.startswith("LIST_NEW")
                or line.startswith("LIST_NEW_I64")
                or line.startswith("LIST_NEW_STR")
            ):
        ...
        value_type_tracker = (
            {}
        )  # Track what type of value each stack position/local holds
        explicit_const_locals = (
            set()
        )  # Track locals that have been assigned via STORE_CONST_*
        for line in bytecode_lines:
            line = line.strip()
            if line.startswith(".func"):
                parts = line.split()
            ...
            if (
                opcode == "LIST_NEW"
                or opcode == "LIST_NEW_I64"
                or opcode == "LIST_NEW_STR"
            ):
                ...
                if len(parts) > 1:
                    ...
                    if struct_type is not None:
                        ...
                        type_stack_sim.append(
                            "i64"
                        )  # default when we don't know struct type
                else:
                    if type_stack_sim:
                        type_stack_sim.pop()
                    type_stack_sim.append("i64")
                ...
                if (
                    len(type_stack_sim) >= 2
                    and type_stack_sim[-1] == "i32"
                    and type_stack_sim[-2] == "i32"
                ):
                ...
                if len(type_stack_sim) >= 3 and (
                    len(type_stack_sim) - 3 in value_type_tracker
                    and value_type_tracker[len(type_stack_sim) - 3] == "str"
                ):
                ...
                if type_stack_sim:
                    ...
                    if (
                        ...
                        and type_stack_sim[-1] == "i32"
                        and type_stack_sim[-2] == "i32"
                        and len(type_stack_sim) - 1 in value_type_tracker
                        and value_type_tracker[len(type_stack_sim) - 1] == "str"
                    ):
                        ...
                        if (
                            idx
                            not in self.functions.get(self.current_function, {}).get(
                                "locals", {}
                            )
                            and idx not in explicit_const_locals
                        ):
                            ...
                            if stack_pos in value_type_tracker:
                                self.local_value_types[idx] = value_type_tracker[
                                    stack_pos
                                ]
                ...
                if len(type_stack_sim) >= 4 and all(
                    t == "i32" for t in type_stack_sim[-4:]
                ):
                    # String contains
                    for _ in range(4):
                        type_stack_sim.pop()
                ...
                if len(parts) >= 2:
                    ...
                    # Handle user-defined functions
                        ...
                        call_return_type = self.functions[call_func_name].get(
                            "return_type", "void"
                        )
                        param_count_call = len(
                            self.functions[call_func_name].get("params", [])
                        )
                ...
                for i in range(0, len(parts) - 1, 2):
                    if (
                        ...
                        if src_idx in local_inferred_types:
                            ...
                            local_inferred_types[dst_idx] = local_inferred_types[
                                src_idx
                            ]
            ...
            # Binary arithmetic operations
            ...
            elif opcode == "LOAD2_DIV_F64":
                # Loads 2 vars (f64), performs division, pushes f64
                type_stack_sim.append("f64")
                ...
                # SWITCH_JUMP_TABLE consumes the switch value and branches; no value is produced.
                if type_stack_sim:
                    type_stack_sim.pop()
    ...
    def _compile_function_body(self, func_name: str, bytecode_lines: List[str]):
        ...
        for i, line in enumerate(func_lines):
            target = None
            if (
                line.startswith("JUMP_IF_FALSE ")
                or line.startswith("JUMP_IF_TRUE ")
                or line.startswith("JUMP ")
            ):
                parts = line.split()
                if len(parts) > 1:
                    target = parts[1]
        ...
        for label in labels_in_func:
            if (
                label.startswith("for_start")
                or label.startswith("forin_start")
                or label.startswith("while_start")
            ):
                ...
                if end_label:
                    ...
                    for lbl in labels_in_func:
                        ...
                        if (
                            lbl.startswith("for_continue")
                            or lbl.startswith("forin_continue")
                            or lbl.startswith("while_continue")
                        ) and start_pos < lbl_pos < end_pos:
                            continue_label = lbl
                            break
        ...
        labels_needing_blocks_outside_loops = (
            set()
        )  # Labels that need blocks opened at function start
        labels_needing_blocks_inside_loops = (
            set()
        )  # Labels that need blocks opened inside loops
        ...
        pre_label_lines = (
            func_lines[:first_label_idx] if first_label_idx is not None else func_lines
        )
        post_label_lines = (
            func_lines[first_label_idx:] if first_label_idx is not None else []
        )

        # Open blocks for labels that are outside loops (at function level)
        blocks_to_open_upfront = sorted(
            labels_needing_blocks_outside_loops,
        ...
        for raw_line in pre_label_lines:
            ...
            try:
                ...
                raise WasmCompilerError(
                    f"Error compiling instruction '{stripped}': {e}"
                )

        loop_stack = (
            []
        )  # Stack of (loop_start, loop_end, continue_label, internal_blocks)
                ...
                if label_name in loop_structures:
                        ...
                        if loop_start_pos < lbl_pos < loop_end_pos:
                            ...
                            for nested_loop_start in loop_structures:
                                if nested_loop_start == label_name:
                                    continue
                                nested_start_pos = label_positions.get(
                                    nested_loop_start, -1
                                )
                                nested_end_label = loop_structures[nested_loop_start][0]
                                nested_end_pos = label_positions.get(
                                    nested_end_label, len(func_lines)
                                )
                    ...
                    loop_stack.append(
                        (
                            ...
                            end_label,
                            continue_label,
                            [lbl for lbl, _ in labels_in_this_loop],
                        )
                    )
                    continue
            ...
            if stripped.startswith("JUMP "):
                ...
                if len(parts) > 1:
                    ...
                    for j in range(len(loop_stack) - 1, -1, -1):
                        loop_start, loop_end, continue_label, internal_blocks = (
                            loop_stack[j]
                        )
                        if loop_start == target_label:
                            ...
                            for k in range(len(active_blocks) - 1, -1, -1):
                                if (
                                    active_blocks[k][0] == loop_start
                                    and active_blocks[k][1]
                                ):  # is_loop
                                    active_blocks.pop(k)
                                    indent -= 1
                                    self.emit(")", indent)
    ...
    def _compile_instruction(self, inst: str, indent: int):
        ...
        # Keep compilation quiet; tests capture output.
        ...
        if opcode == "ADD_CONST_I64":
            ...
            self.type_stack.append("i32")
            if (
                hasattr(self, "extended_type_stack")
            ...
            if (
                ...
                self.emit_comment(
                    "String length: swap and convert i32 len to i64", indent
                )
                # Stack: ... ptr len
            ...
            # Stack (from compiler): [message, condition]
            # Message is either:
            # - NULL sentinel: i64 0
            # - a string pair: (i32 ptr, i32 len)

            # Normalize condition to i32 (0/1)
            if self.type_stack and self.type_stack[-1] == "i64":
            ...
            # Save condition in $temp
            self.emit("local.set $temp", indent)
            if self.type_stack:
                self.type_stack.pop()

            # Decide message shape from compile-time stack types
            has_str_msg = (
                len(self.type_stack) >= 2
                and self.type_stack[-1] == "i32"
                and self.type_stack[-2] == "i32"
            ...
            has_null_msg = (
                not has_str_msg
                and len(self.type_stack) >= 1
                and self.type_stack[-1] == "i64"
            )

            # Stash message into locals and clear it off the value stack.
            # This avoids relying on implicit stack carry-through across structured control flow.
            if has_str_msg:
                # Stack top: ... ptr len
                self.emit("local.set $temp_i32_0", indent)  # len
                self.emit("local.set $temp2", indent)  # ptr
                # Pop (ptr,len)
                if self.type_stack:
                    self.type_stack.pop()
                if self.type_stack:
            ...
            # if (cond == 0) { print message; exit(1); unreachable }
            self.emit("local.get $temp", indent)
            self.emit("i32.eqz", indent)
            self.emit("if", indent)

            if has_str_msg:
                self.emit("local.get $temp2", indent + 1)
                self.emit("local.get $temp_i32_0", indent + 1)
                self.emit("call $println", indent + 1)
                self.imports.add("println")
            else:
                default_msg = "Assertion failed"
                offset = self.add_string_constant(default_msg)
                self.emit(f"i32.const {offset}", indent + 1)
                self.emit(f'i32.const {len(default_msg.encode("utf-8"))}', indent + 1)
                self.emit("call $println", indent + 1)
                self.imports.add("println")

            self.emit("i32.const 1", indent + 1)
            self.emit("call $exit_process", indent + 1)
            self.imports.add("exit_process")
            self.emit("unreachable", indent + 1)

            self.emit("end", indent)

        elif opcode == "EXIT":
            # Exit with status code from stack
            # Native bytecode uses i64; WASM host import expects i32.
            if self.type_stack and self.type_stack[-1] == "i64":
            ...
            if self.type_stack:
                self.type_stack.pop()
            # Tell the validator this path does not continue.
            ...
            if func_name in builtin_map:
                ...
                if (
                    func_name in ("set_timeout", "set_interval")
                        ...
                        if stack_type == "i32":
                            ...
                            ops.append(
                                (
                                    "i32",
                                    temp,
                                    lambda t=temp: self.emit(f"local.get {t}", indent),
                                )
                            )
                            param_idx -= 1
                            stack_idx -= 1
                            ...
                            if param_idx > 0 and param_types[param_idx - 1] == "i32":
                                ...
                                if (
                                    hasattr(self, "extended_type_stack")
                                    and self.extended_type_stack
                                    and stack_idx < len(self.extended_type_stack)
                                    and self.extended_type_stack[stack_idx] == "str"
                                ):
                                    should_unpack = True
                        ...
                        while remaining_args > 0:
                            ...
                            if top_type == "i32":
                                ...
                                if (
                                    ...
                                    if (
                                        stack_idx < len(self.extended_type_stack)
                                        and self.extended_type_stack[stack_idx] == "str"
                                    ):
                                        # Check if previous is also str (ptr)
                                        if (
                                            stack_idx - 1 >= 0
                                            and self.extended_type_stack[stack_idx - 1]
                                            == "str"
                                        ):
                                            is_str = True
                ...
                for i in range(param_count - 1, -1, -1):
                    ...
                    if len(self.type_stack) >= abs(stack_offset):
                        actual_type = self.type_stack[
                            stack_offset
                        ]  # Get the correct stack position
            ...
            # Equality comparison; use f64.eq when operands are floats
            if len(self.type_stack) >= 2 and (
                "f64" in (self.type_stack[-1], self.type_stack[-2])
            ):
            ...
            # Stack: struct_ptr, value -> writes field and leaves struct_ptr on stack
            field_idx = int(args[0])

            # Determine struct type from extended stack (struct ptr is just below value)
            struct_type = None
            if hasattr(self, "extended_type_stack") and self.extended_type_stack:
                for k in range(len(self.extended_type_stack) - 1, -1, -1):
                    vt = self.extended_type_stack[k]
                    if isinstance(vt, str) and vt.startswith("struct:"):
                        val = vt.split(":", 1)[1]
                        if val.isdigit():
                            struct_type = int(val)
                        else:
            ...
            field_type = None
            if struct_type is not None and struct_type in self.struct_defs:
                sdef = self.struct_defs[struct_type]
                fts = sdef.get("field_types", [])
                if field_idx < len(fts):
                    field_type = fts[field_idx]
            ...
            if field_type == "str":
                ...
                self.emit("local.tee $temp_i32_0", indent)  # keep struct ptr
                self.emit("local.get $temp_i32_0", indent)
                self.emit(f"i32.const {offset}", indent)
                self.emit("i32.add", indent)
                ...
                self.emit("local.tee $temp_i32_0", indent)
                self.emit("local.get $temp_i32_0", indent)
                self.emit(f"i32.const {offset}", indent)
                self.emit("i32.add", indent)
                self.emit("local.get $temp_f64", indent)
                ...
                # Default store slot as i64 (covers int/bool/list/set/struct ptr)
                if self.type_stack and self.type_stack[-1] == "i32":
                    self.emit("i64.extend_i32_u", indent)
                    self.type_stack[-1] = "i64"
                self.emit("local.set $temp_i64", indent)
            ...
            # Keep struct type annotation on the remaining pointer
            if (
                hasattr(self, "extended_type_stack")
                and self.extended_type_stack is not None
            ):
            ...
            for string_content in strings:
                ...
                self.emit(
                    f"i32.const {offset}  ;; string: {string_content[:20]}...", indent
                )
            ...
            try:
                const_f = float(const_val)
            except Exception:
                const_f = None

            if const_f == 0.0:
                # If we're inside a try that catches ZeroDivisionError, branch to its handler.
                handler_label = None
                if self.try_stack:
                    for catch_type, lbl in reversed(self.try_stack):
                        if catch_type == "ZeroDivisionError":
                            handler_label = lbl
                            break
                ...
                if handler_label:
                    ...
                    # After unconditional branch, stack is unreachable from this point.
                    self.type_stack = TypeStack(self)
                    self.extended_type_stack = []
                else:
                    # Pop dividend for internal stack accounting; execution will terminate.
                    if self.type_stack:
                        self.emit("drop", indent)
                        self.type_stack.pop()

                    empty_off = self.add_string_constant("")
                    msg = "float division by zero"
                    msg_off = self.add_string_constant(msg)
                    line_num = int(getattr(self, "current_line", 0) or 0)
                    col_num = 0
                    if self._source_lines_for_columns and 0 < line_num <= len(
                        self._source_lines_for_columns
                    ):
                        col_num = len(self._source_lines_for_columns[line_num - 1]) + 1

                    self.emit(f"i32.const {empty_off}", indent)
                    self.emit("i32.const 0", indent)
                    self.emit(f"i32.const {msg_off}", indent)
                    self.emit(f"i32.const {len(msg.encode('utf-8'))}", indent)
                    self.emit(f"i32.const {line_num}", indent)
                    self.emit(f"i32.const {col_num}", indent)
                    self.emit("call $runtime_error", indent)
                    self.emit("unreachable", indent)
                    self.imports.add("runtime_error")

                    # Keep compiler stack consistent (polymorphic after unreachable)
                    self.type_stack.append("f64")
            else:
                if self.type_stack and self.type_stack[-1] == "i64":
                    self.emit("f64.convert_i64_s", indent)
            ...
            # Check divisor == 0.0 before division.
            # IMPORTANT: don't conditionally consume an outer-stack value inside an `if`,
            # otherwise WASM validation can fail. We spill both operands to locals first.
            if (
                len(self.type_stack) >= 2
                and self.type_stack[-1] == "f64"
                and self.type_stack[-2] == "f64"
            ):
                # Stack: ... dividend divisor
                self.emit("local.set $temp_f64", indent)  # store divisor
                self.type_stack.pop()

                # Store dividend bits in i64 temp so we can restore later
                ...
                # Restore dividend and divisor for the actual division
                self.emit("local.get $temp_i64", indent)
            ...
            const_val = args[0]
            # Subtract a constant from the top value
            if self.type_stack and self.type_stack[-1] == "f64":
                self.emit(f"f64.const {const_val}", indent)
            ...
            # Stack: ... set_ptr value
            # String values are (ptr,len) and must be packed into i64
            is_string = False
            if (
                ...
                if hasattr(self, "extended_type_stack") and self.extended_type_stack:
                    if (
                        len(self.extended_type_stack) >= 2
            ...
            if is_string:
                ...
                # Ensure set pointer is i32 and value is i64
            ...
            # Generic membership test: element in container
            # - string contains: (hay_ptr,hay_len, needle_ptr,needle_len) -> i32
            # - set contains: (set_ptr, value_i64) -> i32
            # - list contains: (list_ptr, value_i64) -> i32

            # Detect string contains using extended type info
            if (
                len(self.type_stack) >= 4
                and all(t == "i32" for t in self.type_stack[-4:])
                and hasattr(self, "extended_type_stack")
                and self.extended_type_stack is not None
                and len(self.extended_type_stack) >= 4
                and all(et == "str" for et in self.extended_type_stack[-4:])
            ):
            ...
            if len(self.type_stack) < 2:
                self.emit_comment("CONTAINS - stack underflow", indent)
                return
            ...
            if (
                ...
                container_kind = self.extended_type_stack[-2]
            if container_kind not in ("list", "set"):
                container_kind = getattr(self, "_last_i32_source", None)

            # If element is a string (ptr,len), pack it into i64 (len<<32 | ptr)
            is_string_element = (
                len(self.type_stack) >= 3
                and self.type_stack[-1] == "i32"
                and self.type_stack[-2] == "i32"
            ...
            # (s_ptr,s_len, old_ptr,old_len, new_ptr,new_len) -> (ptr,len)
            ...
            # (s_ptr,s_len, sep_ptr,sep_len) -> list_ptr
            ...
            # ENCODE consumes (string ptr,len) + (encoding ptr,len) and returns bytes.
            # WASM backend treats bytes as a (ptr,len) pair; drop encoding and keep the original string.
            if len(self.type_stack) >= 4:
                # Drop encoding len/ptr
                self.emit("drop", indent)
                self.emit("drop", indent)
                self.type_stack.pop()
                self.type_stack.pop()
            ...
            # No args -> i64 pid (0 in child)
            self.emit("call $fork", indent)
            self.type_stack.append("i64")
            self.imports.add("fork")

        elif opcode in ("JOIN", "WAIT"):
            # pid(i64) -> status(i64)
            if self.type_stack and self.type_stack[-1] == "i32":
                self.emit("i64.extend_i32_u", indent)
                self.type_stack[-1] = "i64"
            self.emit("call $wait", indent)
            if self.type_stack:
                self.type_stack.pop()
            self.type_stack.append("i64")
            self.imports.add("wait")
            ...
            # DECODE consumes (bytes ptr,len) + (encoding ptr,len) and returns string.
            # Since bytes are represented as (ptr,len), this is also a no-op besides dropping encoding.
            if len(self.type_stack) >= 4:
                self.emit("drop", indent)
                self.emit("drop", indent)
                self.type_stack.pop()
                ...
                if (
                    ...
                    if self.extended_type_stack:
                        self.extended_type_stack.pop()
                # Mark remaining (ptr,len) as string
                if (
                    len(self.type_stack) >= 2
                    and hasattr(self, "extended_type_stack")
                    and self.extended_type_stack is not None
            ...
            # Convert top value to i64
            # String: (i32 ptr, i32 len) -> i64
            if (
                len(self.type_stack) >= 2
                and self.type_stack[-1] == "i32"
                and self.type_stack[-2] == "i32"
            ...
            # Convert top value to boolean (i32 0/1)
            if (
                len(self.type_stack) >= 2
                and self.type_stack[-1] == "i32"
                and self.type_stack[-2] == "i32"
            ):
                # strings: bool(s) is true if len != 0
                self.emit("i32.const 0", indent)
                self.emit("i32.ne", indent)
                # Stack is now: ptr, bool. Drop ptr and keep bool.
                self.emit("local.set $temp", indent)
                self.emit("drop", indent)
                self.emit("local.get $temp", indent)
                self.type_stack.pop()  # len
                self.type_stack.pop()  # ptr
                self.type_stack.append("i32")
            elif self.type_stack and self.type_stack[-1] == "i64":
                self.emit("i64.const 0", indent)
                self.emit("i64.ne", indent)
            ...
            if self.type_stack:
                ...
                if top_type == "i64":
                    ...
                    self.emit("local.tee $temp_f64", indent)
                    self.emit("local.get $temp_f64", indent)
                    self.type_stack.append("f64")
                else:
            ...
            if len(args) % 2 == 1:
                ...
                if self.current_function:
                    ...
                    if final_idx < param_count:
                        ...
                        local_type_fr = self.local_vars.get(
                            final_idx - param_count, "i64"
                        )
                else:
                    local_type_fr = "i64"
            ...
            # TRY_BEGIN "ErrorType" handler_label
            catch_type = args[0] if args else ""
            if catch_type.startswith('"') and catch_type.endswith('"'):
                catch_type = catch_type[1:-1]
            handler_label = args[1] if len(args) > 1 else None
            if handler_label:
                self.try_stack.append((catch_type, handler_label))
            ...
            # RAISE "ErrorType" "Message" (message may contain spaces)
            ...
            error_type = strings[0] if len(strings) > 0 else ""
            message = strings[1] if len(strings) > 1 else ""

            handler_label = None
            if self.try_stack:
                for catch_type, lbl in reversed(self.try_stack):
                    if catch_type == error_type:
                        handler_label = lbl
                        break
            ...
            if handler_label:
                ...
                et = self._unescape_string(error_type)
                msg = self._unescape_string(message)
                et_ptr = self.add_string_constant(et)
                msg_ptr = self.add_string_constant(msg)
                self.emit(f"i32.const {et_ptr}", indent)
                self.emit(f"i32.const {len(et.encode('utf-8'))}", indent)
                self.emit(f"i32.const {msg_ptr}", indent)
                self.emit(f"i32.const {len(msg.encode('utf-8'))}", indent)
                self.emit(f"i32.const {int(getattr(self, 'current_line', 0))}", indent)
                self.emit("i32.const 0", indent)
                self.emit("call $runtime_error", indent)
                self.emit("unreachable", indent)
                self.type_stack = TypeStack(self)
            ...
            if (
                ...
                and self.type_stack[-1] == "i32"
                and self.type_stack[-2] == "i32"
                and self.type_stack[-3] == "i64"
                and self.type_stack[-4] == "i32"
            ):
            ...
            # LIST_POP consumes list_ptr (i32) and returns (list_ptr i32, value i64)
            self._ensure_top_is_i32(indent)
            ...
            for arg in args:
                ...
                if self.current_function:
                    ...
                    if var_idx < param_count:
                        ...
                        if (
                            local_idx in self.local_vars
                            and self.local_vars[local_idx] == "str"
                        ):
                            is_str = True
            ...
            # STORE_CONST_I64 slot val [slot val ...]
            ...
            # STORE_CONST_F64 slot val [slot val ...]
            for i in range(0, len(args), 2):
                if i + 1 >= len(args):
                    break
                slot = int(args[i])
                val = args[i + 1]
                var_ref = self._get_var_ref(slot)
                self.emit(f"f64.const {val}", indent)
                self.emit(f"local.set {var_ref}", indent)
            ...
            for i in range(0, len(args), 2):
                ...
                slot = int(args[i])
                val = args[i + 1]
                v = "1" if val in ("1", "true", "True") else "0"
                var_ref = self._get_var_ref(slot)
                self.emit(f"i32.const {v}", indent)
                self.emit(f"local.set {var_ref}", indent)

        elif opcode == "STORE_CONST_STR":
            # STORE_CONST_STR slot "literal" (may contain spaces)
            rest = inst[len("STORE_CONST_STR") :].strip()
            parts = rest.split(maxsplit=1)
            if not parts:
                return
            slot = int(parts[0])
            string_part = parts[1] if len(parts) > 1 else ""

            strings = []
            j = 0
            while j < len(string_part):
                if string_part[j] == '"':
                    k = j + 1
                    while k < len(string_part) and string_part[k] != '"':
                        if string_part[k] == "\\":
                            k += 2
                        else:
                            k += 1
                    if k < len(string_part):
                        strings.append(string_part[j + 1 : k])
                        j = k + 1
                    else:
                        break
                else:
                    j += 1
            s = self._unescape_string(strings[0] if strings else "")
            offset = self.add_string_constant(s)
            var_ref = self._get_var_ref(slot)
            self.emit(f"i32.const {offset}", indent)
            self.emit(f"local.set {var_ref}", indent)
            self.emit(f"i32.const {len(s.encode('utf-8'))}", indent)
            self.emit(f"local.set {var_ref}_len", indent)
            ...
            # Load two variables and divide them as f64 (with div-by-zero semantics handled by DIV_F64)
            var1 = int(args[0])
            var2 = int(args[1])
            var_ref1 = self._get_var_ref(var1)
            var_ref2 = self._get_var_ref(var2)
            ...
            self.emit(f"local.get {var_ref1}", indent)
            # Best-effort type tracking (params/locals may be f64 or i64)
            t1 = "i64"
            if self.current_function:
                func_meta = self.functions.get(self.current_function, {})
                param_count = len(func_meta.get("params", []))
                if var1 < param_count:
                    t1 = self._map_type_to_wasm(func_meta["params"][var1][1])
                else:
                    rel1 = var1 - param_count
                    t1 = self._map_type_to_wasm(self.local_vars.get(rel1, "i64"))
            self.type_stack.append(t1)
            ...
            if self.current_function:
                ...
                param_count = len(func_meta.get("params", []))
                if var2 < param_count:
                    t2 = self._map_type_to_wasm(func_meta["params"][var2][1])
                else:
                    rel2 = var2 - param_count
                    t2 = self._map_type_to_wasm(self.local_vars.get(rel2, "i64"))
            self.type_stack.append(t2)

            # Reuse the DIV_F64 lowering for conversion + div-by-zero + try/propagation.
            self._compile_instruction("DIV_F64", indent)
    ...
    def _emit_call(self, name: str, indent: int):
        ...
        sig = {
            ...
            "str_upper": ["i32", "i32"],
            "str_lower": ["i32", "i32"],
            "str_replace": ["i32", "i32", "i32", "i32", "i32", "i32"],
            "str_contains": ["i32", "i32", "i32", "i32"],
            "str_eq": ["i32", "i32", "i32", "i32"],
...
def compile_to_wasm(
    bytecode: str, source_file: Optional[str] = None
) -> Tuple[str, Dict]:
    """Compile fr bytecode to WebAssembly text format"""
    compiler = WasmCompiler()
    return compiler.compile(bytecode, source_file=source_file)
