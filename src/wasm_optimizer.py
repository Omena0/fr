...
def instr_pat(name: str, args: Optional[str] = None) -> str:
    ...
    name = re.escape(name)
    return r"\(?" + name + r"\s+" + args + r"\)?" if args else r"\(?" + name + r"\)?"
...
def apply_peephole_optimizations(code: str) -> str:
    ...
    code = re.sub(
        instr_pat("f64.const", r"0\.0") + SEP + instr_pat("f64.add"), "", code
    )
    code = re.sub(
        instr_pat("f64.const", r"0\.0") + SEP + instr_pat("f64.sub"), "", code
    )
    ...
    code = re.sub(
        instr_pat("f64.const", r"1\.0") + SEP + instr_pat("f64.mul"), "", code
    )

    # Pattern: double negation for i64
    code = re.sub(
        instr_pat("i64.const", "-1")
        + SEP
        + instr_pat("i64.mul")
        + SEP
        + instr_pat("i64.const", "-1")
    ...
    code = re.sub(
        instr_pat("i64.extend_i32_u") + SEP + instr_pat("i32.wrap_i64"), "", code
    )
    ...
    code = re.sub(
        instr_pat("i32.wrap_i64") + SEP + instr_pat("i64.extend_i32_s"), "", code
    )

    # Pattern: redundant extend/wrap pairs
    code = re.sub(
        instr_pat("i64.extend_i32_s") + SEP + instr_pat("i32.wrap_i64"), "", code
    )
    code = re.sub(
        instr_pat("i64.extend_i32_u") + SEP + instr_pat("i32.wrap_i64"), "", code
    )
...
def optimize_boolean_ops(code: str) -> str:
    ...
    code = re.sub(
        instr_pat("i32.const", "0") + SEP + instr_pat("i32.eq"), "(i32.eqz)", code
    )

    # Pattern: (i64.const 0) (i64.eq) -> (i64.eqz)
    code = re.sub(
        instr_pat("i64.const", "0") + SEP + instr_pat("i64.eq"), "(i64.eqz)", code
    )
    ...
    code = re.sub(
        instr_pat("i32.const", "0") + SEP + instr_pat("i32.and"),
        "(drop)\n(i32.const 0)",
        code,
    )
    ...
    code = re.sub(
        instr_pat("i64.const", "0") + SEP + instr_pat("i64.and"),
        "(drop)\n(i64.const 0)",
        code,
    )
    ...
    code = re.sub(
        instr_pat("i64.const", "-1") + SEP + instr_pat("i64.or"),
        "(drop)\n(i64.const -1)",
        code,
    )
    ...
    code = re.sub(
        instr_pat("i64.lt_s") + SEP + instr_pat("i32.eqz"), "(i64.ge_s)", code
    )
    code = re.sub(
        instr_pat("i64.le_s") + SEP + instr_pat("i32.eqz"), "(i64.gt_s)", code
    )
    code = re.sub(
        instr_pat("i64.gt_s") + SEP + instr_pat("i32.eqz"), "(i64.le_s)", code
    )
    code = re.sub(
        instr_pat("i64.ge_s") + SEP + instr_pat("i32.eqz"), "(i64.lt_s)", code
    )
    code = re.sub(
        instr_pat("i64.lt_u") + SEP + instr_pat("i32.eqz"), "(i64.ge_u)", code
    )
    code = re.sub(
        instr_pat("i64.le_u") + SEP + instr_pat("i32.eqz"), "(i64.gt_u)", code
    )
    code = re.sub(
        instr_pat("i64.gt_u") + SEP + instr_pat("i32.eqz"), "(i64.le_u)", code
    )
    code = re.sub(
        instr_pat("i64.ge_u") + SEP + instr_pat("i32.eqz"), "(i64.lt_u)", code
    )
    ...
    code = re.sub(
        instr_pat("i32.lt_s") + SEP + instr_pat("i32.eqz"), "(i32.ge_s)", code
    )
    code = re.sub(
        instr_pat("i32.le_s") + SEP + instr_pat("i32.eqz"), "(i32.gt_s)", code
    )
    code = re.sub(
        instr_pat("i32.gt_s") + SEP + instr_pat("i32.eqz"), "(i32.le_s)", code
    )
    code = re.sub(
        instr_pat("i32.ge_s") + SEP + instr_pat("i32.eqz"), "(i32.lt_s)", code
    )
    code = re.sub(
        instr_pat("i32.lt_u") + SEP + instr_pat("i32.eqz"), "(i32.ge_u)", code
    )
    code = re.sub(
        instr_pat("i32.le_u") + SEP + instr_pat("i32.eqz"), "(i32.gt_u)", code
    )
    code = re.sub(
        instr_pat("i32.gt_u") + SEP + instr_pat("i32.eqz"), "(i32.le_u)", code
    )
    code = re.sub(
        instr_pat("i32.ge_u") + SEP + instr_pat("i32.eqz"), "(i32.lt_u)", code
    )
...
def apply_strength_reduction(code: str) -> str:
    ...
    code = re.sub(
        instr_pat("i64.const", r"(\d+)") + SEP + instr_pat("i64.mul"),
        _mul_shift_i64,
        code,
    )
    code = re.sub(
        instr_pat("i32.const", r"(\d+)") + SEP + instr_pat("i32.mul"),
        _mul_shift_i32,
        code,
    )
    ...
    code = re.sub(
        instr_pat("i64.const", r"(\d+)") + SEP + instr_pat("i64.div_u"),
        _div_shift_i64_u,
        code,
    )
    code = re.sub(
        instr_pat("i32.const", r"(\d+)") + SEP + instr_pat("i32.div_u"),
        _div_shift_i32_u,
        code,
    )
    ...
    code = re.sub(
        instr_pat("i64.const", r"(\d+)") + SEP + instr_pat("i64.rem_u"),
        _rem_mask_i64,
        code,
    )
    code = re.sub(
        instr_pat("i32.const", r"(\d+)") + SEP + instr_pat("i32.rem_u"),
        _rem_mask_i32,
        code,
    )
...
def optimize_drops(code: str) -> str:
    ...
    code = re.sub(
        instr_pat("f32.const", r"-?[\d.]+(?:e[+-]?\d+)?") + SEP + instr_pat("drop"),
        "",
        code,
    )
    ...
    code = re.sub(
        instr_pat("f64.const", r"-?[\d.]+(?:e[+-]?\d+)?") + SEP + instr_pat("drop"),
        "",
        code,
    )
    ...
    while i < len(lines):
        ...
        if (
            ...
            while i < len(lines):
                ...
                if (
                    next_line.startswith("(block")
                    or next_line.startswith("block")
                    or next_line.startswith("(loop")
                    or next_line.startswith("loop")
        ...
        while i < n:
            ...
            if code[i] == "(":
                ...
                if match:
                    name = match[2]
                    stack.append(
                        {"start": i, "name": name, "content_start": i + len(match[0])}
                    )
                else:
                    stack.append(None)
                i += 1
...
def remove_unused_blocks(code: str) -> str:
    ...
    while i < n:
        ...
        if code[i] == "(":
            if match := re.match(r"\((block|loop)(?:\s+(\$[\w\d_]+))?", code[i:]):
                type_ = match.group(1)
                name = match.group(2)
...
def remove_duplicate_local_gets(code: str) -> str:
    pat = instr_pat("local.get", r"(\$\w+)") + SEP + instr_pat("local.get", r"\1\b")
    return re.sub(pat, r"local.get \1", code)


def remove_overwritten_local_sets(code: str) -> str:
    pat = instr_pat("local.set", r"(\$\w+)") + SEP + instr_pat("local.set", r"\1\b")
    return re.sub(pat, r"local.set \1", code)


def replace_tee_drop_with_set(code: str) -> str:
    pat = instr_pat("local.tee", r"(\$\w+)") + SEP + instr_pat("drop")
    return re.sub(pat, r"local.set \1", code)


def remove_drop_after_call(code: str) -> str:
    pat = instr_pat("call", r"(\$\w+)") + SEP + instr_pat("drop")
...
def inline_trivial_if(code: str) -> str:
    pat = r"\(if" + SEP + r"\(then" + SEP + r"([\s\S]*?)" + SEP + r"\)\s*\)"
    return re.sub(pat, r"\1", code)
...
def mark_i64_high_half(code: str) -> str:
    return re.sub(
        instr_pat("i64.extend_i32_u")
        + SEP
        + instr_pat("i64.const", "32")
        + SEP
        + instr_pat("i64.shl"),
        "(;; PACK32_HIGH ;;)",
        code,
