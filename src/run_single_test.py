...
def extract_error_message(error_text):
    ...
    # Check if there's an "Exception: ExceptionType" line for raise statements
    # Format:
    # Exception: RuntimeError
    #   File "..." line 4 in main
    #   <unknown>:4: Test error message
    exc_type = None
    lines = error_text.strip().split('\n')
    for line in lines:
        if line.startswith('Exception: '):
            exc_name = line.split('Exception: ', 1)[1].strip()
            # Only use it if it's a specific type, not generic "Runtime Error" or "Syntax Error"
            if exc_name not in ('Runtime Error', 'Syntax Error', 'Parse Error'):
                exc_type = exc_name
            break
            ...
            if len(parts) > 1:
                ...
                if first_colon != -1:
                    ...
                    if second_colon != -1:
                        ...
                        # Add exception type if found
                        if exc_type:
                            message = f"[{exc_type}] {message}"
                        # Return in format ?line,char:message
                        return f"?{line_num},{char_num}:{message}"

        # Try to match "<unknown>:line:char: Message" format (from native compiler)
        if '<unknown>:' in line and ':' in line:
            parts = line.split('<unknown>:', 1)
            if len(parts) > 1:
                loc_and_msg = parts[1]
                first_colon = loc_and_msg.find(':')
                if first_colon != -1:
                    ...
                    if second_colon != -1:
                        ...
                        # No char position, just line:message
                        message = rest.strip()
                        if exc_type:
                            message = f"[{exc_type}] {message}"
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if result.returncode != 0:
                    ...
                    # If there's an error, treat it as error test (discard stdout)
                    # This matches Python/VM runtime behavior for error tests
                    if native_error:
                        native_output = None
