...
def apply_instr_semantics(
    instr: str, args: List[str], stack: List[str], locals_types: Dict[int, str]
) -> List[str]:
    ...
    if op.startswith("ADD_CONST_"):
        ...
        return s
    if (
        op.startswith("SUB_CONST_")
        or op.startswith("MUL_CONST_")
        or op.startswith("DIV_CONST_")
        or op.startswith("MOD_CONST_")
    ):
