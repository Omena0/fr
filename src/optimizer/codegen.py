...
class CodeGen:
    ...
    def _get_fall_through_block(
        self, func: Function, block: BasicBlock
    ) -> BasicBlock | None:
    ...
    def _emit_block(self, func: Function, block: BasicBlock, alloc: RegAllocation):
        ...
        for inst in block.instructions:
            ...
            save_regs = (
                self._get_caller_saves(inst) if inst.op in self._CALL_OPS else []
            )
    ...
    def _emit_instruction(
        self, func: Function, block: BasicBlock, inst: Instruction, alloc: RegAllocation
    ):
        ...
        if op == Op.TO_STR:
            ...
            if isinstance(a_type, ListType):
                ...
                self._emit_runtime_call(
                    "runtime_float_to_str", inst, alloc, 1, float_args=[0]
                )
        ...
        if op in (Op.SIN, Op.COS, Op.TAN):
            ...
            self._emit_runtime_call(
                name, inst, alloc, 1, float_args=[0], float_ret=True
            )
            return
        ...
        if op == Op.FLOOR:
            self._emit_runtime_call(
                "runtime_floor", inst, alloc, 1, float_args=[0], float_ret=True
            )
            return

        if op == Op.CEIL:
            self._emit_runtime_call(
                "runtime_ceil", inst, alloc, 1, float_args=[0], float_ret=True
            )
            return

        if op == Op.ROUND:
            self._emit_runtime_call(
                "runtime_round", inst, alloc, 1, float_args=[0], float_ret=True
            )
            return

        if op == Op.POW:
            self._emit_runtime_call(
                "runtime_pow", inst, alloc, 2, float_args=[0, 1], float_ret=True
            )
            return
    ...
    def _emit_float_binop(self, inst: Instruction, alloc: RegAllocation, mnemonic: str):
    ...
    def _emit_shift(self, inst: Instruction, alloc: RegAllocation, mnemonic: str):
        ...
        shift_val = (
            _get_const_value(inst.operands[1]) if len(inst.operands) > 1 else None
        )
    ...
    def _emit_parallel_int_moves(self, moves: list[tuple[str, str]]):
        ...
        while remaining:
            ...
            for i, (src, dst) in enumerate(remaining):
                ...
                conflict = any(
                    src2 == dst for j, (src2, _) in enumerate(remaining) if j != i
                )
                if not conflict:
