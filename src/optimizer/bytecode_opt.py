...
class BytecodeOptimizer:
    ...
    def optimize(self, bytecode: str) -> str:
        ...
        lines = self._apply_pass_preserve_lines(
            lines, self.merge_fused_load_store_sequences
        )
        ...
        lines = self._apply_pass_preserve_lines(
            lines, self.optimize_comparison_inversions
        )
        ...
        lines = self._apply_pass_with_lines(
            lines, self.eliminate_redundant_continue_labels
        )
    ...
    def fuse_repeated_arith_store(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if i + 3 < len(lines):
                ...
                if (
                    a.startswith("LOAD ")
                    and (b.startswith("CONST_I64 ") or b.startswith("CONST_F64 "))
                    and d.startswith("STORE ")
                ):
                    ...
                    if typ is not None:
                        ...
                        while j + 3 < len(lines):
                            ...
                            # same type and op
                            if typ == "I64" and not (
                                b2.startswith("CONST_I64 ") and c2 == f"{op}_I64"
                            ):
                                break
                            if typ == "F64" and not (
                                b2.startswith("CONST_F64 ") and c2 == f"{op}_F64"
                            ):
                                break
                        ...
                        if len(blocks) >= 2:
                            ...
                            result.append(
                                f"{indent}FUSED_ARITH_CONST {op} {typ} {len(blocks)} {args}"
                            )
                            i = j
                            continue
    ...
    def merge_const_store_init(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            line = lines[i].strip()
            indent = (
                lines[i][: len(lines[i]) - len(lines[i].lstrip())] if lines[i] else "  "
            )

            # Pattern: CONST_I64 X followed by STORE Y
            if (
                i + 1 < len(lines)
                and line.startswith("CONST_I64 ")
                and lines[i + 1].strip().startswith("STORE ")
            ):
                ...
                while (
                    j + 1 < len(lines)
                    and lines[j].strip().startswith("CONST_I64 ")
                    and lines[j + 1].strip().startswith("STORE ")
                ):
            ...
            if (
                ...
                while (
                    j + 1 < len(lines)
                    and lines[j].strip().startswith("CONST_F64 ")
                    and lines[j + 1].strip().startswith("STORE ")
                ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("CONST_BOOL ")
                and lines[i + 1].strip().startswith("STORE ")
            ):
                ...
                while (
                    j + 1 < len(lines)
                    and lines[j].strip().startswith("CONST_BOOL ")
                    and lines[j + 1].strip().startswith("STORE ")
                ):
        ...
        while i < len(lines):
            ...
            if stripped.startswith("STRUCT_GET "):
                ...
                if len(triplet_args) >= 6:  # at least 2 triplets to be worth fusing
                    result.append(
                        f"{indent}FUSED_GET_STORE_LOAD {' '.join(triplet_args)}"
                    )
                    i = j
                    continue
    ...
    def merge_adjacent_labels(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if stripped.startswith("LABEL "):
                ...
                primary = next(
                    (lbl for lbl in labels if lbl.startswith(preferred_prefixes)),
                    labels[0],
                )
        ...
        while i < len(lines):
            ...
            if line.startswith('CONST_STR "'):
                ...
                while lookahead < lookahead_limit:
                    ...
                    if lookahead_line and not lookahead_line.startswith(
                        ("LOAD", "DUP", "CONST_", ".line")
                    ):
                        break
                    lookahead += 1
    ...
    def optimize_variable_copies(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            line = lines[i].strip()
            indent = (
                lines[i][: len(lines[i]) - len(lines[i].lstrip())] if line else "  "
            )

            # Pattern: LOAD x, STORE y
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD ")
                and lines[i + 1].strip().startswith("STORE ")
            ):
    ...
    def combine_patterns(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if (
                i + 3 < len(lines)
                and line.startswith("LOAD ")
                and lines[i + 1].strip() == "CONST_I64 1"
                and lines[i + 2].strip() == "ADD_I64"
                and lines[i + 3].strip().startswith("STORE ")
            ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD ")
                and lines[i + 1].strip().startswith("LOAD ")
            ):
            ...
            if (
                ...
                and line.startswith("LOAD ")
                and lines[i + 1].strip().startswith("LOAD ")
                and lines[i + 2].strip() in _LOAD2_FUSABLE_OPS
            ):
                op = lines[i + 2].strip()
                var1 = line.split()[1]
                var2 = lines[i + 1].strip().split()[1]
            ...
            if line.startswith("LOAD "):
                ...
                if (
                    len(loads) == 2
                    and j < len(lines)
                    and lines[j].strip() in _LOAD2_FUSABLE_OPS
                ):
            ...
            if (
                stripped_line.startswith("LOAD ") and len(stripped_line.split()) == 3
            ):  # LOAD with exactly 2 args
    ...
    def remove_dead_stores(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if not stripped or stripped.startswith(".") or stripped.startswith("#"):
                ...
                i += 1
                continue
            if stripped.startswith("STORE ") and len(stripped.split()) == 2:
                ...
                if var_idx not in loaded_vars:
                    ...
                    while prev_idx >= 0 and (
                        not result[prev_idx].strip()
                        or result[prev_idx].strip().startswith(".")
                        or result[prev_idx].strip().startswith("#")
                    ):
                        prev_idx -= 1
                    if prev_idx >= 0:
    ...
    def optimize_negation(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("CONST_I64 -1")
                and lines[i + 1].strip() == "MUL_I64"
            ):
            ...
            if (
                i + 2 < len(lines)
                and line.startswith("CONST_I64 0")
                and lines[i + 1].strip() == "SWAP"
                and lines[i + 2].strip() == "SUB_I64"
            ):
                result.append(f"{indent}NEG")
                i += 3
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("CONST_F64 -1")
                and lines[i + 1].strip() == "MUL_F64"
            ):
                result.append(f"{indent}NEG")
                i += 2
    ...
    def algebraic_simplifications(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if (
                i + 1 < len(lines)
                and line in ["CONST_F64 0.0", "CONST_F64 0"]
                and lines[i + 1].strip() == "ADD_F64"
            ):
                i += 2
                continue
            ...
            if (
                i + 1 < len(lines)
                and line in ["CONST_F64 1.0", "CONST_F64 1"]
                and lines[i + 1].strip() == "MUL_F64"
            ):
                i += 2
                continue
            ...
            if i + 1 < len(lines) and line == "NOT" and lines[i + 1].strip() == "NOT":
                i += 2
                continue
    ...
    def optimize_comparison_inversions(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD2_CMP_LT ")
                and lines[i + 1].strip() == "NOT"
            ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD2_CMP_GT ")
                and lines[i + 1].strip() == "NOT"
            ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD2_CMP_LE ")
                and lines[i + 1].strip() == "NOT"
            ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD2_CMP_GE ")
                and lines[i + 1].strip() == "NOT"
            ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD2_CMP_EQ ")
                and lines[i + 1].strip() == "NOT"
            ):
            ...
            if (
                i + 1 < len(lines)
                and line.startswith("LOAD2_CMP_NE ")
                and lines[i + 1].strip() == "NOT"
            ):
    ...
    def constant_folding(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            ...
            if (
                i + 2 < len(lines)
                and line.startswith("CONST_I64 ")
                and lines[i + 1].strip().startswith("CONST_I64 ")
            ):
            ...
            if (
                i + 2 < len(lines)
                and line.startswith("CONST_F64 ")
                and lines[i + 1].strip().startswith("CONST_F64 ")
            ):
                op_name = lines[i + 2].strip()
            ...
            if (
                i + 2 < len(lines)
                and line.startswith("CONST_BOOL ")
                and lines[i + 1].strip().startswith("CONST_BOOL ")
            ):
                op_name = lines[i + 2].strip()
                if op_name in ("AND", "OR"):
                    try:
                        ...
                        result.append(
                            f"{indent}CONST_BOOL {'true' if val else 'false'}"
                        )
                        i += 3
                        continue
                    except (ValueError, IndexError):
        ...
        while i < len(lines):
            ...
            if i + 2 < len(lines):
                ...
                if (
                    ...
                    and lines[i + 1].strip().startswith("CMP_EQ_CONST ")
                    and lines[i + 2].strip().startswith("JUMP_IF_TRUE ")
                ):
                    ...
                    while (
                        j + 2 < len(lines)
                        and lines[j].strip() == f"LOAD {switch_var}"
                        and lines[j + 1].strip().startswith("CMP_EQ_CONST ")
                        and lines[j + 2].strip().startswith("JUMP_IF_TRUE ")
                    ):
                        try:
                    ...
                    if len(cases) >= 3:
                        ...
                        if is_dense and len(sorted_cases) >= 5 and default_label:
                            ...
                            result.append(
                                f"{indent}SWITCH_JUMP_TABLE {min_val} {max_val} {labels_str} {default_label}"
                            )
                        else:
            ...
            if line.startswith("LABEL case_"):
                ...
                while j < len(lines) and lines[j].strip().startswith("LABEL case_"):
                    ...
                    try:
                        ...
                        case_num = (
                            int(label_suffix[0]) if label_suffix[0].isdigit() else None
                        )
                        if case_num is None:
                            break
                    except (ValueError, IndexError):
                    ...
                    if j + 4 < len(lines):
                        ...
                        if (
                            body_line1.startswith("LOAD ")
                            and body_line2.startswith("ADD_CONST_I64 ")
                            and body_line3.startswith("STORE ")
                            and body_line4.startswith("JUMP ")
                            and not body_line4.startswith("JUMP_IF")
                        ):
                ...
                if len(cases) >= 3:
                    ...
                    if all(
                        c["var"] == first_var and c["end_label"] == first_end
                        for c in cases
                    ):
                        # Check if add values follow a pattern: case N adds N+1
                        pattern_match = all(
                            c["add_val"] == c["case_num"] + 1 for c in cases
                        )
                        ...
                        if pattern_match:
                            ...
                            if switch_var:
                                ...
                                for k in range(
                                    len(result) - 1, max(0, len(result) - 20), -1
                                ):
                                    if (
                                        result[k]
                                        .strip()
                                        .startswith("SWITCH_JUMP_TABLE ")
                                    ):
                                        ...
                                        default_label = parts[
                                            3 + num_cases
                                        ]  # Last label is default

                                        # All cases point to optimized label, except default
                                        new_labels = [optimized_label] * num_cases + [
                                            default_label
                                        ]
                                        result[k] = (
                                            f"{indent}SWITCH_JUMP_TABLE {min_val} {max_val} {' '.join(new_labels)}"
                                        )
                                        break
            ...
            if line.startswith("LOAD "):
                ...
                for j in range(i + 1, min(i + 6, len(lines))):
                    ...
                    if (
                        ...
                        or next_line.startswith("JUMP")
                        or next_line == f"STORE {var}"
                        or next_line.startswith("STORE_CONST")
                    ):
                        break
                    ...
                    if next_line == f"LOAD {var}":
                        ...
                        for k in range(i + 1, j):
                            ...
                            if any(
                                check_line.startswith(unsafe)
                                for unsafe in unsafe_for_dup
                            ):
                                has_unsafe = True
                                break
    ...
    def eliminate_redundant_continue_labels(self, lines: List[str]) -> List[str]:
        ...
        for line in lines:
            ...
            elif stripped.startswith("JUMP_IF_TRUE ") or stripped.startswith(
                "JUMP_IF_FALSE "
            ):
                parts = stripped.split()
                if len(parts) >= 2:
        ...
        while i < len(lines):
            ...
            if line.startswith("LABEL for_continue"):
                ...
                if label_name not in jumped_labels and i + 2 < len(lines):
                    ...
                    if next1.startswith("INC_LOCAL ") and next2.startswith(
                        "JUMP for_start"
                    ):
                        i += 1
                        continue
        ...
        while i < len(lines):
            ...
            if line == "LIST_NEW":
                ...
                if value_type and (
                    ...
                    or len(values_f64) >= 3
                    or len(values_str) >= 3
                    or len(values_bool) >= 3
                ):
                    if value_type == "i64":
                        result.append(
                            f"{indent}LIST_NEW_I64 {len(values_i64)} {' '.join(values_i64)}"
                        )
                    elif value_type == "f64":
                        result.append(
                            f"{indent}LIST_NEW_F64 {len(values_f64)} {' '.join(values_f64)}"
                        )
                    elif value_type == "str":
                        result.append(
                            f"{indent}LIST_NEW_STR {len(values_str)} {' '.join(values_str)}"
                        )
                    elif value_type == "bool":
                        result.append(
                            f"{indent}LIST_NEW_BOOL {len(values_bool)} {' '.join(values_bool)}"
                        )
            ...
            if line.startswith("JUMP_IF_FALSE "):
                ...
                while j < len(lines):
                    ...
                    if curr.startswith("STORE "):
                        if_store_var = (
                            curr.split()[1] if len(curr.split()) > 1 else None
                        )
                        if_store_idx = j
                ...
                indent = lines[if_store_idx][
                    : len(lines[if_store_idx]) - len(lines[if_store_idx].lstrip())
                ]
                ...
                while cond_start >= len(result):
                    ...
                    if (
                        prev.startswith("LABEL ")
                        or prev.startswith("JUMP ")
                        or prev.startswith(".local ")
                        or prev.startswith("STORE ")
                        or prev.startswith("STORE_CONST_")
                    ):
                        cond_start += 1
                        break
                    cond_start -= 1
