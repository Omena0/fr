        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if any(actual == e.strip() for e in expected_list):
                ...
                return (
                    test_path,
                    "wrong",
                    f"expected={repr(expect)}, got={repr(actual)}",
                )
...
def main():
    ...
    # Collect tests
    test_files = sorted(
        glob.glob(str(REPO_ROOT / "cases" / "**" / "*.fr"), recursive=True)
    )
    ...
    for t in test_files:
        ...
        for pattern in ignore_patterns:
            if (
                ...
                or fnmatch.fnmatch(rel.replace("cases/", ""), pattern)
            ):
                skip = True
                break
        if not skip:
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(run_single_ssa_test, t, runtime_obj): t for t in filtered
        }
        done_count = 0
        total = len(futures)
        for future in as_completed(futures):
            ...
            if done_count % 20 == 0 or done_count == total:
                ...
                failed = (
                    len(results["wrong"])
                    + len(results["ssa_fail"])
                    + len(results["link_fail"])
                    + len(results["error"])
                )
                print(
                    f"\r[{done_count}/{total}] pass={passed} fail={failed} skip={len(results['skip'])}",
                    end="",
                    flush=True,
                )
